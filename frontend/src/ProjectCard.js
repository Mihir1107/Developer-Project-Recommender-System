...
/* ─────────────────────────────────────────
   Inline styles as CSS-var-aware objects
   (no separate .css file needed for the card —
    all card styles live in App.css under .card-*)
───────────────────────────────────────── */
...
const cardInnerStyle = { padding: '20px 22px' };

/* animated left accent bar via pseudo — done with a real div instead */
const AccentBar = () => (
  <div style={{
    position: 'absolute',
    top: 0, left: 0,
...
function ProjectCard({ project, isBookmarked, onToggleBookmark }) {
  ...
  const badgeStyle  = DIFFICULTY_BADGE[difficulty] || DIFFICULTY_BADGE.Beginner;
